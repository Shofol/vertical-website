...
const BlogCard = ({ article }) => {
    ...
    return (
        <div className="rounded-3xl max-w-xs lg:max-w-sm bg-white flex flex-col flex-none mb-6">
            ...
            {article.fields.author.fields.image.fields.file.url && <div className={'w-16 h-16 -mt-7 ml-4 userImgPolygon'} style={{ background: `center / cover url(${article.fields.author.fields.image.fields.file.url}), #fff` }}></div>}
            <div className="px-7 py-5 z-20">
                ...
                <div className="flex items-center my-6 text-sm text-#002B56">
                    {article.fields.author.fields.title && <p>{article.fields.author.fields.title}</p>}
